...
class ServiceProcezorExampleWithSalaryHomeOfficeTest: QuickSpec {
    static func getTargetsFromDb(period: IPeriod) -> Array<ITermTarget> {
        ...
        let targets  = Array<ITermTarget>(arrayLiteral:
            TermTarget(_month: montCode, _contract: contract, _position: position, _variant: variant1,
            ...
            TermTarget(_month: montCode, _contract: contract, _position: position, _variant: variant1,
                _article: ArticleCode.get(ExampleArticleConst.ARTICLE_ALLOWCE_HOFFICE.rawValue),
    ...
    override func spec() {
        describe("Example example - service Procezor", {
            ...
            let restTargets = ServiceProcezorExampleWithSalaryHomeOfficeTest.getTargetsFromDb(period: testPeriod)
            ...
            let testArticles: [Int32] = [80001, 80005, 80002, 80006, 80007, 80010, 80012, 80008, 80009, 80011, 80013]
